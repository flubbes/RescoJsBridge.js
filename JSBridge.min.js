...
(function(){var _scriptVersion=10.2;var _inherit=(function(){function _(){}
return function(child,parent){_.prototype=parent.prototype;child.prototype=new _;child.prototype.constructor=child;child.superproto=parent.prototype;return child;};})();var _addProperty=function(obj,name,writable,value){if(!obj._privVals)
...
request.isOnlineForce=forceMode;var cmdParams=JSON.stringify(request);var self=this;window.MobileCRM.bridge.command('entitysave',cmdParams,function(res){self.id=res.id;self.isNew=false;self.isOnline=res.isOnline;self.primaryName=res.primaryName;self.properties=res.properties;callback.call(self,null);},function(err){callback.call(self,err);},null);return this;};MobileCRM.Metadata.requestObject=function(callback,errorCallback,scope){MobileCRM.bridge.requestObject("Metadata",function(obj){if(obj){MobileCRM.Metadata.entities=obj;}
...
MobileCRM.MetaEntity.prototype.getProperty=function(name){var properties=this.properties;if(properties){for(var i=0;i<properties.length;i++){if(properties[i].name==name)
return properties[i];}}
return null;};MobileCRM.MetaEntity.prototype.canRead=function(){return this.getDepth(0)!=0;}
...
format+=output;this.execute("Offline."+output,success,failed,scope);};MobileCRM.FetchXml.Fetch.deserializeFromXml=function(xml,success,failed,scope){MobileCRM.bridge.invokeStaticMethodAsync("MobileCrm.Data","MobileCrm.Data.FetchXml.Fetch","Deserialize",[xml],success,failed,scope);};MobileCRM.FetchXml.Fetch.prototype.serializeToXml=function(success,failed,scope){var reqParams=JSON.stringifyNotNull({entity:this.entity,page:this.page,count:this.count,aggregate:this.aggregate});MobileCRM.bridge.command('fetchToXml',reqParams,success,failed,scope);};MobileCRM.FetchXml.Entity.prototype.addAttribute=function(name,alias,agreggate){var attr=new MobileCRM.FetchXml.Attribute(name);if(alias)
attr.alias=alias;if(agreggate)
attr.aggregate=agreggate;this.attributes.push(attr);return attr;};MobileCRM.FetchXml.Entity.prototype.addAttributes=function(){this.allattributes=true;};MobileCRM.FetchXml.Entity.prototype.addFilter=function(){var filter=this.filter;if(filter)
...
MobileCRM.Platform.email=function(address,subject,body,errorCallback,scope){var emailContent={Address:address,Subject:subject,Body:body};_executePlatformAction(1,emailContent,null,errorCallback,scope);};MobileCRM.Platform.emailWithAttachments=function(address,subject,body,attachments,entity,relationship,errorCallback,scope){var params={relationship:relationship,body:body,subject:subject,address:address,attachments:attachments,entity:entity}
MobileCRM.bridge.command("sendEmailWithAttachments",JSON.stringify(params),null,errorCallback,scope);};MobileCRM.Platform.openUrl=function(url){var param={Address:url};_executePlatformAction(3,param,null,null);};MobileCRM.Platform.getURL=function(success,failed,scope){MobileCRM.bridge.command("getURL",null,success,failed,scope);};MobileCRM.Platform.getDeviceInfo=function(success,failed,scope){MobileCRM.bridge.command("getDeviceInfo",null,success,failed,scope);};MobileCRM.Platform.navigateTo=function(latitude,longitude,failed,scope){var params={Address:latitude+","+longitude};_executePlatformAction(4,params,null,null);};MobileCRM.Platform.scanBarCode=function(success,failed,scope){MobileCRM.bridge.command("scanBarcode",null,success,failed,scope);};MobileCRM.Platform.getLocation=function(success,failed,scope,age,precision,timeout){var params={maxAge:age,accuracy:precision};if(timeout)
...
_inherit(MobileCRM.UI._DetailView,MobileCRM.ObservableObject);MobileCRM.UI._DetailView.prototype.getItemByName=function(name){var items=this.items;var nItems=items.length;for(var i=0;i<nItems;i++){var item=items[i];if(item.name==name)
return item;}
...
params.OutputFile=outputFile;MobileCRM.bridge.command("runMobileReportAsync",JSON.stringify(params),success,failed,scope);};MobileCRM.MobileReport.showForm=function(entityName,source,fetchXml,failed,scope){var params={fetchXml:fetchXml,entityName:entityName,source:source};MobileCRM.bridge.command("showMobileReportForm",JSON.stringify(params),null,failed,scope);};MobileCRM.Questionnaire.showForm=function(id,failed,scope){var params={id:id};MobileCRM.bridge.command("showQuestionnaire",JSON.stringify(params),null,failed,scope);};_inherit(MobileCRM.UI.IFrameForm,MobileCRM.ObservableObject);MobileCRM.UI.IFrameForm.showModal=function(caption,url,options){MobileCRM.bridge.command("showIFrame",JSON.stringify({caption:caption,url:url,modal:true,options:options}));};MobileCRM.UI.IFrameForm.show=function(caption,url,maximized,options){MobileCRM.bridge.command("showIFrame",JSON.stringify({caption:caption,url:url,modal:false,maximized:maximized,options:options}));};MobileCRM.UI.IFrameForm.requestObject=function(callback,errorCallback,scope){MobileCRM.bridge.requestObject("IFrameForm",function(obj){if(callback.call(scope,obj)!=false){var changed=obj.getChanged();return changed;}
...
MobileCRM.bridge.invokeMethodAsync("EntityList",method,ps,null,errorCallback,scope);};MobileCRM.UI.EntityList.clickCell=function(rowIndex,cellIndex,errorCallback,scope){MobileCRM.bridge.invokeMethodAsync("EntityList","StartListEdit",[rowIndex,cellIndex,MobileCRM.UI.EntityListCellAction.Clickable,null],null,errorCallback,scope);};MobileCRM.UI.EntityList.setDataSource=function(dataSource){MobileCRM.UI.EntityList._dataSource=dataSource;};MobileCRM.UI.EntityList._initDataSource=function(chunkReadyCmdId,fetch){var result={};var dataSource=MobileCRM.UI.EntityList._dataSource;if(dataSource){dataSource._chunkReadyCmdId=chunkReadyCmdId;dataSource.fetch=fetch;result={chunkSize:dataSource.chunkSize||50};}
return JSON.stringify(result);};MobileCRM.UI.EntityList._loadNextChunk=function(page,count){var ds=MobileCRM.UI.EntityList._dataSource;if(ds&&typeof ds.loadNextChunk=="function"){try{var fetch=ds.fetch;fetch.page=page;fetch.count=count;ds.loadNextChunk(page,count);return"OK";}catch(e){return e.message;}}
...
return id;}}
else if(typeof CrmBridge!=="undefined"){MobileCRM.Bridge.prototype.command=function(command,params,success,failed,scope){var cmdId=this._createCmdObject(success,failed,scope);CrmBridge.println(cmdId+';'+command+':'+params);};if(typeof CrmBridge.invoke!=="undefined"){MobileCRM.Bridge.prototype.invoke=function(command,params){var result=CrmBridge.invoke(command,params);if(result.length>=4&&result.substr(0,4)=='ERR:')
throw new MobileCrmException(result.substr(4));else
return eval('('+result+')');};}
MobileCRM.bridge=new MobileCRM.Bridge('Android');}
else{if(typeof window.external!=="undefined"){var win10=typeof window.win10version!=="undefined";if(typeof window.external.notify!=="undefined"||win10){MobileCRM.Bridge.prototype.command=function(command,params,success,failed,scope){var cmdId=this._createCmdObject(success,failed,scope);window.external.notify(cmdId+';'+command+':'+params);};MobileCRM.Bridge.prototype.invoke=function(command,params){var result=undefined;var error=null;var cmdId=this._createCmdObject(function(res){result=res;},function(err){error=err;},this);window.external.notify(cmdId+';'+command+':'+params);if(error)
throw new MobileCrmException(error);else
return result;};if(typeof window.alert==="undefined"){window.alert=function(text){MobileCRM.bridge.invoke("alert",text);};}
...
else if(typeof window.external.ProcessCommand!=="undefined"){MobileCRM.Bridge.prototype.command=function(command,params,success,failed,scope){var cmdId=this._createCmdObject(success,failed,scope);window.external.ProcessCommand(cmdId,command,params);};if(typeof window.external.InvokeCommand!=="undefined"){MobileCRM.Bridge.prototype.invoke=function(command,params){var result=window.external.InvokeCommand(command,params);if(result.length>=4&&result.substr(0,4)=='ERR:')
throw new MobileCrmException(result.substr(4));else
return eval('('+result+')');};}
MobileCRM.bridge=new MobileCRM.Bridge('Windows');}}
else if(typeof webkit!=="undefined"&&typeof webkit.messageHandlers!=="undefined"&&typeof webkit.messageHandlers.JSBridge!=="undefined"){MobileCRM.Bridge.prototype.command=function(command,params,success,failed,scope){var cmdId=this._createCmdObject(success,failed,scope);var cmdText=cmdId+';'+command+':'+params;webkit.messageHandlers.JSBridge.postMessage(cmdText);};MobileCRM.bridge=new MobileCRM.Bridge('iOS');}
